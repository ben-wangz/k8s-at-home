		...
		<div style={{ display: 'grid', gap: 18 }}>
			...
			<div style={surfaceStyle}>
				...
				<div style={{ overflowX: 'auto' }}>
					...
					{projects.length > 0 && <table style={tableStyle}><thead><tr>{['Project', 'Slug', 'Open', 'In Progress', 'In Review', 'Updated', 'Actions'].map((header) => <th key={header} style={tableHeadStyle}>{header}</th>)}</tr></thead><tbody>{projects.map((project) => { const active = project.slug === selectedProject; return <tr key={project.id} style={{ background: active ? palette.accentSoft : 'transparent' }}><td style={tableCellStyle}><div style={{ fontWeight: 700 }}>{project.name}</div><div style={{ color: palette.muted, fontSize: 13, marginTop: 4 }}>{project.summary}</div></td><td style={{ ...tableCellStyle, fontFamily: 'ui-monospace, SFMono-Regular, monospace' }}>{project.slug}</td><td style={tableCellStyle}>{project.open}</td><td style={tableCellStyle}>{project.inProgress}</td><td style={tableCellStyle}>{project.inReview}</td><td style={tableCellStyle}>{project.updatedAt}</td><td style={tableCellStyle}><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><button style={inlineActionButtonStyle} onClick={() => onSelectProject(project.slug)}>Open Tasks</button><button style={inlineGhostActionButtonStyle} onClick={() => navigator.clipboard.writeText(project.id)}>Copy ID</button></div></td></tr> })}</tbody></table>}
				</div>
			</div>
		</div>
	)
