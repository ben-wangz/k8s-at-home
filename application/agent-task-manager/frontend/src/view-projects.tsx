...
import { CopyFeedbackButton, EmptyState, WorkspaceHeader, palette } from './view-shared'
			...
			<div style={surfaceStyle}>
				<div style={toolbarStyle}>
					...
					<div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
						...
						/>
						<button style={ghostActionButtonStyle} onClick={onSearch}>
							GO
						</button>
					</div>
				</div>
				<div style={{ overflowX: 'auto' }}>
					{projects.length === 0 && (
						<EmptyState
							...
							detail="Refine the project name query or clear it to see all projects sorted by project name."
						/>
					)}
					{projects.length > 0 && (
						<table style={tableStyle}>
							<thead>
								<tr>
									{['Project', 'Slug', 'Open', 'In Progress', 'In Review', 'Done', 'Cancelled', 'Updated', 'Actions'].map((header) => (
								...
								{projects.map((project) => {
									...
									return (
										<tr key={project.id} style={{ background: active ? palette.accentSoft : 'transparent' }}>
											...
											<td style={tableCellStyle}>{project.done}</td>
											<td style={tableCellStyle}>{project.cancelled}</td>
											...
											<td style={tableCellStyle}>
												<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
													...
													<CopyFeedbackButton value={project.id} label="Copy ID" style={inlineGhostActionButtonStyle} />
												</div>
											</td>
										</tr>
									)
